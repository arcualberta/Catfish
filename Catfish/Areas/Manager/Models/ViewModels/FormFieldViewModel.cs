...
namespace Catfish.Areas.Manager.Models.ViewModels
{
    public class FormFieldViewModel : KoBaseViewModel
    {
        ...
        public List<CFTextValue> Name { get; set; }
        //public string Description { get; set; }
        public List<CFTextValue> Description { get; set; }
        public bool IsRequired { get; set; }
        public bool IsOptionField { get; set; }
        public List<CFTextValue> MultilingualOptionSet { get; set; }
        ...
        public FormFieldViewModel(FormField formField, int abstractFormId)
        {
            ...
            if (IsOptionField)
            {
                MultilingualOptionSet = new List<CFTextValue>();
                ...
                foreach (var lang in ConfigHelper.Languages)
                    MultilingualOptionSet.Add(new CFTextValue(lang.TwoLetterISOLanguageName, lang.NativeName, ""));

                IReadOnlyList<Option> options = (formField as OptionsField).Options;
                foreach (Option op in options)
                {
                    foreach (CFTextValue txt in op.Value)
                    {
                        CFTextValue editorData = MultilingualOptionSet.Where(x => x.LanguageCode == txt.LanguageCode).FirstOrDefault();

                        //Accommodating odd situations where data has a language that is not specified in the configuration
                        if (editorData == null)
                            MultilingualOptionSet.Add(editorData = new CFTextValue(txt.LanguageCode, txt.LanguageLabel, ""));
        ...
        public FormField InstantiateDataModel()
        {
            ...
            if (typeof(OptionsField).IsAssignableFrom(type))
            {
                ...
                foreach (CFTextValue optionValueSet in MultilingualOptionSet)
                {
                    ...
                    for (int i = 0; i < optionTextValues.Length; ++i)
                    {
                        ...
                        optList[i].Value.Add(new CFTextValue(optionValueSet.LanguageCode, optionValueSet.LanguageLabel, optionTextValues[i]));
                    }
                }
